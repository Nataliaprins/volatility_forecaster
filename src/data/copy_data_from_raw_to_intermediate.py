...
def copy_data_from_raw_to_intermetidate(root_dir):
    ...
    raw_files = os.listdir(os.path.join(root_dir, "raw"))

    # Process each file in raw/
    for raw_file in raw_files:
        # Read the file
        df = pd.read_csv(
            os.path.join(root_dir, "raw", raw_file),
            parse_dates=True,
            index_col=0,
            encoding='utf-8'
        )
        ...
        # Save the file
        df.to_csv(
            os.path.join(root_dir, "intermediate", raw_file),
            index=True,
        )
...
if __name__ == '__main__':
    copy_data_from_raw_to_intermetidate(
        root_dir =  os.path.abspath('data/yahoo')
    )